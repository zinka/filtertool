...
def ft_import(dtype="complex"):
    ...
    file_path = tkdlg.askopenfilename(
        title="Open file", filetypes=[("txt file", ".csv"), ("All files", ".*")]
    )
    master.quit()
...
def s_to_w(poly_ip, coef_norm=False):
    ...
    if len(poly_ip) == 1:
        ...
        #         poly_op = sp.simplify(poly_ip.subs(s, 1j * w))  # simplification issue
        poly_op = poly_ip.subs(s, 1j * w)  # substitution
        poly_op = sp.Poly(poly_op, w).all_coeffs()
        poly_op = I_to_i(poly_op)
        poly_op = np.reshape(poly_op, (len(poly_op), -1))
        if coef_norm:
            poly_op = poly_op / poly_op[0]
    return poly_op
...
def w_to_s(poly_ip, coef_norm=False):
    ...
    if len(poly_ip) == 1:
        ...
        #         print(poly_ip.subs(w, -1j * s))
        #         print(powsimp(poly_ip.subs(w, -1j * s)))
        #         poly_op = sp.simplify(poly_ip.subs(w, -1j * s))  # substitution

        poly_op = poly_ip.subs(w, -1j * s)  # substitution
...
def Chebyshev_gen(N, poles):
        ...
        for j in range(1, len(poles)):
            if np.isinf(poles[j]):
                ...
                U = (
                    x * tmp1
                    -tmp1 / poles[j]
                    +sp.sqrt(x ** 2 - 1) * np.sqrt(1 - 1 / poles[j] ** 2) * tmp2
                )
                V = (
                    x * tmp2
                    -tmp2 / poles[j]
                    +sp.sqrt(x ** 2 - 1) * np.sqrt(1 - 1 / poles[j] ** 2) * tmp1
                )
                tmp1 = sp.simplify(U)
                tmp2 = sp.simplify(V)
        U = tmp1
    ...
    Num = (
        np.reshape(Num, (len(Num), -1)) / norm
    )  # so that abs(polynomial) value becomes 1 at +1
    return Num, Den
...
def plot_mag(
    eps, eps_R, F, P, E, w_min=-2, w_max=2, w_num=500, dB=True, dB_limit=-40, plot=True
):
    r"""
...
def plot_delay(roots_E, w_min=-2, w_max=2, w_num=500, plot=True):
    ...
    for i in range(len(w)):
        ...
        for j in range(len(roots_E)):
            tmp += -roots_E[j].real / (
                roots_E[j].real ** 2 + (w[i] - roots_E[j].imag) ** 2
            )
        tau[i] = tmp
    if plot:
        plt.plot(w, tau)
...
def MN_to_Sparam(
    M, Rs, Rl, w_min=-2, w_max=2, w_num=500, dB=True, dB_limit=-40, plot=True
):
    r"""
...
def MN2_to_Sparam(
    M, Rs=1, Rl=1, w_min=-2, w_max=2, w_num=500, dB=True, dB_limit=-40, plot=True
):
    r"""
    ...
    if p == q:
        if p == i:
            ...
            tht = np.arctan(
                (
                    -M[i - 1, j - 1]
                    +np.sqrt(M[i - 1, j - 1] ** 2 - M[i - 1, i - 1] * M[j - 1, j - 1])
                    / M[j - 1, j - 1]
                )
            )
        else:
        ...
        if p == i and q == j:
            tht = 0.5 * np.arctan(
                2 * M[i - 1, j - 1] / (M[j - 1, j - 1] - M[i - 1, i - 1])
            )
        elif p == j and q == i:
            k = p
            tht = 0.5 * np.arctan(
...
def dual_band(F, P, E, eps, eps_R=1, x1=0.5, map_type=1):
    ...
    if (map_type == 1) or (map_type == 2):
        ...
        #         F1 = sp.simplify(F.subs(s, s1)) # issue with simplify
        F1 = F.subs(s, s1)
        F1 = sp.Poly(F1, s).all_coeffs()
        F1 = I_to_i(F1)

        E = sp.Poly(E.ravel().tolist(), s)
        #         E1 = sp.simplify(E.subs(s, s1)) # issue with simplify
        E1 = E.subs(s, s1)
        E1 = sp.Poly(E1, s).all_coeffs()
        E1 = I_to_i(E1)

        P = sp.Poly(P.ravel().tolist(), s)
        #         P1 = sp.simplify(P.subs(s, s1)) # issue with simplify
        P1 = P.subs(s, s1)
...
if __name__ == "__main__":
    ...
    # ==========================================================================
    # 4th order example (P. 278, Sec. 8.4.4, R. J. Cameron et al.)
    # ==========================================================================
...
#     plot_mag(eps, eps_R, F, P, E, w_min=-8, w_max=8, dB_limit=-50)

    # ==============================================================================
    # 7th order filter, Cameron, Sec. 8.3.1.2, page 265
    # ==============================================================================
...
#     MN_to_Sparam(M, Rs, Rl, w_min=-10, w_max=10, w_num=500, dB=True, dB_limit=-100)
    ...
    # ==========================================================================
    # 4th order fully canonical filter (P. 276, Sec. 8.4.3, R. J. Cameron et al.)
    # ==========================================================================
...
# TODO write a pdf article for conventional filter theory ... how gn values obtained and how LPPs are converted to immitance inverter based LPPs, etc
# TODO also add Richard transformations, Kuroda's identities, a few lpf and bpf filter techniques
